...
-- Независимые отпуска (не привязаны к квартальному плану).
-- Нужны, чтобы отпуск мог пересекать границы кварталов.
CREATE TABLE beer_tracker.vacations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    board_id INTEGER NOT NULL,
    member_id TEXT NOT NULL,
    member_name TEXT NOT NULL,
...
CREATE INDEX idx_vacations_board ON beer_tracker.vacations(board_id);
CREATE INDEX idx_vacations_member ON beer_tracker.vacations(member_id);
CREATE INDEX idx_vacations_dates ON beer_tracker.vacations(start_date, end_date);
